...
def write_info_file(
  ...
  if info.get('url_info', url_info) != url_info:
    raise ValueError(
        'File info {} contains a different checksums that the downloaded one: '
        'Stored: {}; Expected: {}'.format(
            info_path, info['url_info'], url_info))
  info = dict(
      urls=list(urls),
      dataset_names=list(set(dataset_names)),
      original_fname=original_fname,
      url_info=url_info.asdict(),
  )
