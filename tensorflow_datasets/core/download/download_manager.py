...
class DownloadManager(object):
  ...
  def _save_url_info_and_rename(
    ...
    # TODO(tfds): As downloads are cached even without checksum, we could
  ...
  def _download(self, resource: Union[str, resource_lib.Resource]):
    ...
    # File can be located in two different locations:
    ...
    # To decide whether to download / reuse file, and file location, compute:
    # * `existing_path`: Contains `file_path` or `url_path` if the file was
    #   already downloaded. `None` otherwise
    ...
    if not self._force_download:
      ...
      if url in self._url_infos:
        ...
        if resource.exists_locally(file_path):
      ...
      if not existing_path and resource.exists_locally(url_path):
