...
class DownloadManagerTest(testing.TestCase):
  ...
  through python structure:

    files: Dict[str, str], mapping existing files (downloaded, extracted
      and `.INFO`) -> file content
  ...
  # ----- Virtual tf.io.gfile API -----

  def _add_file(self, path, content=None) -> None:
    self.files[path] = f'Content of {path}' if content is None else content

  def _list_directory(self, path):
    return [
        os.path.basename(p) for p in self.files
  ...
  def _rename(self, from_, to, overwrite=False):
    if not overwrite and to in self.files:
      raise FileExistsError(f'Cannot overwrite: {from_} -> {to}')
    if from_ not in self.files:
      raise FileNotFoundError(f'Cannot rename unknown file: {from_}')
    self.files[to] = self.files.pop(from_)

  # ----- Downloader/Extractor patch -----

  def _download(self, url, tmpdir_path):
    self.downloaded_urls.append(url)
    # If the name isn't explicitly provided, then it is extracted from the url.
    filename = self.dl_fnames.get(url, os.path.basename(url))
    # Save the file in the tmp_dir
    self._add_file(os.path.join(tmpdir_path, filename))
    return promise.Promise.resolve(self.dl_results[url])

  def _extract(self, path, method, dest):
    self.extracted_paths.append(path)
    self._add_file(dest, f'Extracted dir from {path}')
  ...
  def setUp(self):
    ...
    # Virtual file system
    self.files = {}  # Dict file_path -> file_content
    ...
    absltest.mock.patch.object(
        downloader._Downloader, 'download', side_effect=self._download).start()
    absltest.mock.patch.object(
        extractor._Extractor, 'extract', side_effect=self._extract).start()
  ...
  def test_download(self):
    ...
    # File `a` is cached
    self._add_file(a.file_path)
    self._add_file(a.file_path + '.INFO')
    # INFO file of c has been deleted:
    self._add_file(c.file_path)
  ...
  def test_extract(self):
    ...
    self._add_file('/extract_dir/ZIP.cached')
  ...
  def test_download_cached(self):
    ...
    self.assertCountEqual(self.downloaded_urls, [a.url])
    self.assertCountEqual(self.files, [a.url_path, a.url_path + '.INFO'])
    ...
    self.assertCountEqual(self.files, [a.file_path, a.file_path + '.INFO'])
    ...
    self.assertCountEqual(self.files, [
        a.url_path,
        a.url_path + '.INFO',
